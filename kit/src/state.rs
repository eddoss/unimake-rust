...
pub fn clone(vm: &VirtualMachine) -> PyResult<State> {
    let dict = vm.builtins.get_attr("__unimake__", vm)?;
    Ok(utils::cast::<py::Object>(vm, dict)?.state.read().clone())
}

pub fn read<F>(vm: &VirtualMachine, func: F) -> PyResult<()>
where
    F: FnOnce(&State) -> PyResult<()>,
{
    let states = clone(vm)?;
    func(&states)?;
    vm.builtins
...
where
    F: FnOnce(&mut State) -> PyResult<()>,
{
    let mut states = clone(vm)?;
    func(&mut states)?;
    vm.builtins
        .set_attr("__unimake__", py::Object::from(states).to_pyobject(vm), vm)
}
