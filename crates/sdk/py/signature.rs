...
impl Argument {
    fn new(name: String, kind: Kind, index: usize) -> Self {
        Self {
            name,
            kind,
            index,
    ...
    pub fn is_keyword(&self) -> bool {
        matches!(self.kind, Kind::Keyword)
    }

    pub fn is_variadic(&self) -> bool {
        matches!(self.kind, Kind::Variadic)
    }

    pub fn is_variadic_keyword(&self) -> bool {
        matches!(self.kind, Kind::VariadicKeyword)
    }
}
...
impl Signature {
    pub fn of(callable: PyObjectRef, vm: &VirtualMachine) -> Result<Self> {
        ...
        let func = if callable.payload_is::<PyFunction>() {
            ...
            match callable.downcast::<PyFunction>() {
                Ok(v) => v,
                Err(_) => {
                    return Err(Error::Python(vm.new_runtime_error(
        ...
        code.varnames.iter().enumerate().for_each(|(index, name)| {
            ...
            result.args.push(Argument::new(name, Kind::Regular, index));
        });

        let set = |result: &mut Self, name: String, kind: Kind| match result
            .args
            .iter_mut()
        ...
        for (kind, names) in [
            (Kind::Positional, args.posonlyargs),
            (Kind::Keyword, args.kwonlyargs),
        ] {
        ...
        for (kind, name) in [
            (Kind::Variadic, args.vararg),
            (Kind::VariadicKeyword, args.varkwarg),
        ] {
